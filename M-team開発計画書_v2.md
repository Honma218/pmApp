...
> **【スコープ変更に伴う注記・2026-07-11】** 本書は「既存の業務アプリ（Repo Base）を新スタックで
> 再実装する」旧スコープの計画書であり、**現行スコープ（スライス進捗集計アプリのみの開発）とは
> 一致しない。** 特に技術選定（Express／NestJS／Java・Spring 移行）・受け入れテスト（`acceptance/`・
> 参照モック・golden）に関する記述は現行スコープでは使用しない。
>
> 汎用的な運用判断（層境ゲート・役割分担・モデル配分・Flywheel 等）は引き続き参考になるが、
> 正本は `CLAUDE.md`・`docs/adr/` および `2026-07-11_slice-progress-aggregator_実装ロードマップ.md`。
> 本書は履歴記録として残し、内容の改訂は行っていない（PM 判断待ち。`docs/memory-bank/` 参照）。
